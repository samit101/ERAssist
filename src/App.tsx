...
export default function App() {
  ...
  return <main className='max-w-md mx-auto p-3 space-y-3 pb-10'>
    <header className='card flex items-center justify-between relative'>
      ...
      {menuOpen && <><button className='fixed inset-0 z-10' onClick={() => setMenuOpen(false)} aria-label='Close menu' /><div className='absolute left-0 top-12 z-20 bg-white border rounded-xl shadow p-2 w-44 space-y-1'><button className={`w-full text-left p-2 rounded ${page === 'board' ? 'bg-slate-100' : ''}`} onClick={() => nav('board')}>Main Board</button><button className={`w-full text-left p-2 rounded ${page === 'newCase' ? 'bg-slate-100' : ''}`} onClick={() => nav('newCase')}>New Case</button><button className={`w-full text-left p-2 rounded ${page === 'admin' ? 'bg-slate-100' : ''}`} onClick={() => nav('admin')}>Admin</button></div></>}
    </header>

    {page === 'board' && <>
      <HeaderSummary seen={state.patientSeenCount} target={state.patientTarget} threadsDue={dueTasks.length} setSeen={(n) => setState((s) => ({ ...s, patientSeenCount: n }))} />
